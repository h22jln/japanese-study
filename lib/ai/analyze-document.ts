...
아래에 제공된 PDF 추출 텍스트 전체를 분석하세요. bodyLines는 원문 본문을 최대한 보존하고, 단어와 문법만 학습자가 복습하기 좋은 형태로 정리하세요.

가장 중요한 원칙:
- bodyLines는 요약하거나 축약하지 말 것
- PDF 추출 텍스트에 있는 일본어 본문, 예문, 질문, 대화문을 가능한 한 누락 없이 원문 순서대로 담을 것
- 문장이 길어도 의미 단위로만 줄을 나누고, 내용을 짧게 고쳐 쓰거나 합쳐서 줄이지 말 것
- 선택지/문제 번호/표 제목도 일본어 독해나 복습에 필요하면 남길 것
- 명백한 머리말·꼬리말·페이지 번호·깨진 OCR 잡음만 제외할 것
...
- 프리토킹/질문형에서도 bodyLines에는 자료의 일본어 질문과 답변용 표현을 누락 없이 원문 순서대로 담고, vocabulary에는 주제 핵심어와 말하기 표현을 우선 추출
...
- bodyLines: 화면에 보여줄 일본어 원문을 한 줄씩 담은 배열
- 본문/대화형 자료에서는 일본어 본문 흐름을 원문 순서대로 보존하기
- 문법 핸드아웃형 자료에서는 문법 패턴 제목과 일본어 예문을 누락 없이 담되, 완전히 같은 줄만 중복 제거하기
- 프리토킹/질문형 자료에서는 일본어 질문과 답변에 바로 쓸 수 있는 표현을 원문 순서대로 담기
- 한국어 번역만 있는 줄은 제외하되, 일본어와 함께 붙어 있는 한국어 설명은 본문 이해에 필요하면 남겨도 됨
- 명백한 머리말·꼬리말·페이지 번호만 제외
...
- PDF 추출 결과에는 줄바꿈 노이즈가 있을 수 있으니 줄 나눔만 자연스럽게 보정하고, 내용 자체는 줄이거나 요약하지 말 것`;
...
- 본문 아래나 옆의 작은 글씨 각주, 별표 설명, 괄호 주석, 어휘 풀이가 원문 이해에 필요하면 절대 버리지 말고 bodyLines 끝에 원문 순서대로 포함
- 각주는 본문과 구분되도록 "※ ..." 또는 "주: ..." 형태로 시작하되, 사진에 보이는 일본어/한국어 설명 내용을 최대한 그대로 보존
- 각주 안에 있는 핵심 단어도 vocabulary 추출 대상에 포함
